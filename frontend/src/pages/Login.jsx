...
function Login() {
    ...
    const fillAndLogin = async (user, pass) => {
        setUsername(user);
        setPassword(pass);
        setLoading(true);
        try {
        ...
        <div className="min-h-screen flex flex-col justify-center items-center p-4 md:p-6 font-sans relative overflow-hidden" style={{ backgroundColor: '#0c0d12' }}>
            ...
            <div className="w-full max-w-4xl h-auto md:h-[520px] flex flex-col md:flex-row shadow-2xl rounded-3xl overflow-hidden border border-slate-900 relative z-10 bg-slate-950">
                ...
                <div className="w-full md:w-[42%] flex flex-col justify-between p-8 relative min-h-[420px] md:min-h-0" style={{ backgroundColor: 'rgba(255,255,255,0.97)' }}>
                    ...
                    <div className="my-auto flex flex-col items-center space-y-6 pt-4">
                        {/* Avatar */}
                        <div className="w-16 h-16 rounded-full flex items-center justify-center text-white shadow-md" style={{ background: 'linear-gradient(135deg, #334155, #64748b)' }}>
                            <User className="w-8 h-8 stroke-[1.5]" />
                        </div>

                        <form onSubmit={handleLogin} className="w-full space-y-4 px-1">
                            {/* Username */}
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full pl-11 pr-4 py-2.5 rounded-full border border-slate-300 focus:border-[#18264F] focus:ring-1 focus:ring-[#18264F] outline-none text-slate-900 text-xs font-bold placeholder:text-slate-400 tracking-wider transition"
                                    value={username}
                            ...
                            {/* Options */}
                            <div className="flex justify-between items-center text-[9px] text-slate-400 font-bold px-1 select-none">
                                <label className="flex items-center gap-1.5 cursor-pointer">
                                    <input
                                        type="checkbox"
                                        className="w-3.5 h-3.5 text-[#18264F] border-slate-300 rounded focus:ring-0 cursor-pointer"
                                    />
                                    <span>Remember me</span>
                                </label>
                                <a 
                                    href="#forgot" 
                                    onClick={(e) => { e.preventDefault(); showToast("Password recovery is not available in demo mode."); }} 
                                    className="hover:underline transition hover:text-[#18264F]"
                                >
                ...
                >
                    ...
                    <div className="flex justify-between items-center w-full z-10 select-none relative">
                        ...
                        <Link to="/register" className="px-4 py-1.5 rounded-full border border-white hover:bg-white hover:text-slate-900 transition text-[9px] font-black uppercase tracking-wider select-none">
                            Sign Up
                        </Link>
                    </div>
                    ...
                    <div className="my-auto max-w-sm pl-2 space-y-4 z-10 relative">
                        ...
                        <div className="text-[9px] pt-1 font-bold text-slate-500">
                            Not a member? <Link to="/register" className="underline text-slate-300 hover:text-white transition">Sign up now</Link>
                        </div>
                    </div>
